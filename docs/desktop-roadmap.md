...
**A third pass, at `d716047`, re-checked every claim this document makes about
*this tree* — not the research, which is about crates and a protocol and does
not move, but the file:line citations, the counts, and the sequencing.** Two
stages had shipped in the meantime and were still written as plans; one symbol
the sequencing told a future commit to fix had been fixed and then deleted
outright; several line numbers had drifted by a few hundred lines. Every
in-repo `file:line` below was re-resolved by grep, and where a stage's state is
asserted it is asserted in the table under *Sequencing* with the command that
established it beside it. **The vendored-crate citations were not re-read** — they are
...
at `Cargo.toml:113-114`. Verified as `unsafe fn` in the pinned sources:
...
src/
  ...
  action.rs      NEW. enum Action { Go, Refresh, ToggleNav, NewChat, ... }
                 ...
                 and six into viewport::refresh_named (viewport.rs:423), which
...
**Where each stage stands, established from the tree at `d716047` and from
`git log`, because two of these shipped after the section below was written and
reading it as a plan would have been reading it wrong.** Each stage carries its
verdict inline; the state was checked rather than remembered, and how is stated
with it.
...
tables, collapse eight `cfg`s, delete ~25 lines of prose that now describes
nothing. Two lines off the CI iOS job: `cargo check --package goose-mobile
--target aarch64-apple-ios` becomes the whole command. *Compresses to one
person; it is one commit and splitting it produces only conflicts.* All four
gates were run on this edit and pass.

The unrelated thing that run turned up and that this stage was told to carry —
`FULLSCREEN_CLASS` not being `cfg`-gated while its only caller was, so the iOS
check emitted a `dead_code` warning under `ci.yml`'s workspace-wide
`RUSTFLAGS: -D warnings` — **was fixed and has since been deleted outright.**
`8acddd8` gated the constant; `ece4857` then removed the whole
...
simulator through its keyed states, and its docstring is explicit that a run
REPLACES the gallery, because a state carried over from another branch keeps
passing `docs/audit.js` forever while the markup no longer exists. N agents each
adding a screen cannot each re-capture. That argument stands and the freshness
guarantee is unchanged — what changed is that a capture may now be **several
logs in one invocation**, which is how both shells write into one store without
...
shape — a rule nothing renders and nothing triggers looks exactly like a rule
that works: `ece4857` (fullscreen read off `tao`, and the one-title-per-window
invariant given a check) and `d716047` (the band's title given back the room
`.conn-badge` was spending at the rail tier). The prose for all of it is at the
end of `docs/design.md`; this document should not grow a second copy.

**Stage 3 — the protocol backlog. UNTOUCHED. Compresses almost perfectly.** A wrapper is
...
**Stage 4 — `src/platform/` and `Action`, with a test. UNTOUCHED.** The
...
**Stage 6 — bundling and entitlements. UNTOUCHED. NOT COMPRESSIBLE (Apple's
clock).** `Dioxus.toml`'s `[bundle]` is still `identifier` and `publisher` and
nothing else — there is no plist hook, so `CFBundleURLTypes` (deep links) and
...
measurement rather than as an intention. A capability that cannot make the same
three statements has changed the phone.
