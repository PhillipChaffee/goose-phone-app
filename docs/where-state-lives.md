...
- **That `session/load` replays before the RPC resolves.** Observable from a client since #308, and not yet observed against a real one. The notifications do arrive on a separate task, but "which came first" is a question a client can answer about itself: `src/timing.rs` marks the request, the first fold for that session and the reply, and a debug build prints all three as `@@TIME@@` lines. Against `crates/mock-goose-server/` the first frame lands 0.04–0.19 ms ahead of the reply over three runs — which establishes that the ordering is possible and nothing at all about a server with a conversation to re-read. Until that run happens, believe `load_session.rs:269-329` and don't treat it as measured.
