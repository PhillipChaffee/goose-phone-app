...
- **That side effects of already-run tools remain on disk with no transcript
  ...
  run does not support it. The experiment that would settle it is written and
  checked in — `scripts/verify/side-effect-experiment.sh` — and has not been
  run. See 7.6.
...
the model works through several rounds of tool calls, touching the disk as it
goes; one hits an approval gate; you lock the phone. The round that was open is
lost the same way — the assistant message, every tool request in it, and every
tool response in it — because none of it is persisted until `agent.rs:3339`.
Earlier rounds and your prompt survive.

An earlier draft of this paragraph said "the model opens a round with several
tool calls; the auto-approved ones start running and touch the disk". Reading
the source, that shape does not happen: approved tools are dispatched into
`tool_futures` (`agent.rs:865-910`) but not *polled* until the approval stream
has finished (`agent.rs:2654-2683`), and that stream awaits each confirmation in
turn (`tool_execution.rs:183`). A tool approved in the same LLM message as one
that is still asking should therefore never execute. That is a source read, of
exactly the kind section 0 falsified twice, so 7.6 runs it as the `batched` case
...
The experiment is written and checked in:
`scripts/verify/side-effect-experiment.sh`, on the same harness as section 0
(`crates/goose-acp-client/examples/perm_loss.rs`, modes `sideeffect` and
`readback`). It has **not been run**. What follows is what it does and what
each outcome would mean; nothing below is a measurement.

**7.6.1 How it gets a completed tool and a pending ask into the same turn.**
Not by asking nicely — goose's ordering decides which shapes are even possible,
and the two that matter behave differently:

- **Approved and asking in the SAME LLM message.** Approved tools are
  dispatched into `tool_futures`
  (`/Users/phillipchaffee/git/goose/crates/goose/src/agents/agent.rs:865-910`),
  but those futures are not *polled* until the approval stream has finished
  (`agent.rs:2654-2683`), and `handle_approval_tool_requests` awaits each
  confirmation one at a time
  (`/Users/phillipchaffee/git/goose/crates/goose/src/agents/tool_execution.rs:183`).
  Reading that source, a tool approved alongside a tool that is still asking
  never executes at all.
- **Approved in an EARLIER round of the same turn.** A round's messages are
  persisted at the bottom of each loop iteration (`agent.rs:3339`, in the loop
  opened at `:2331`). Reading that source, a tool that finished in round 1 was
  already written to the session file before round 2 blocked.

So the script runs both, `sequential` and `batched`, and reports which one the
model actually produced rather than which one it was asked for. Both readings
above are source reads, and section 0 falsified two source reads; that is the
whole reason this is a script.

**7.6.2 The mechanics.** Fresh session, `mode = approve`, real goose over ws,
one turn, two tool calls. Two nonces: one in the probe's **path**, one in its
**contents**. The harness answers the *first* ask — the one whose `raw_input`
carries the path nonce — with `allow_once`, so that tool runs to completion and
lands the file; it never answers the second, and parks. `kill -STOP` only, since
section 0 already measured that STOP and KILL agree about the round and STOP is
the shape a suspended phone has. Then 75 seconds, then `perm_loss inspect`.

**7.6.3 The two observations, and both have to be made.** First the transcript:
does the replay contain the write at all? The test for that is *not* a grep for
the probe nonce — the user's prompt names the probe path and the content nonce,
and the user's prompt is exactly what section 0 found survives, so a nonce grep
returns true in the world where nothing survived. The discriminator is whether
any `ToolCall` update was replayed, which in section 0 was zero of them.

Then the file, on the server's filesystem. There are two routes to it and the
script prefers the first:

1. `PROBE_CHECK_CMD='ssh <host> cat'` — authoritative, and the only one that
   involves no model.
2. A `readback` session otherwise. Note the trap in the original design was
   asking **the session under test**, because that starts a new turn on the
   history being measured. A *separate* throwaway session has its own history
   and does not, so this route is legitimate — but it is still a model
   reporting on a file, so it is nonce-guarded: the readback prompt names the
   path nonce and is never told the content nonce. A model that invents a
   successful read cannot invent the content nonce. No content nonce in the
   answer means believe nothing and use route 1.

**7.6.4 What each outcome would mean.**

| file on disk | write in replay | reading |
| --- | --- | --- |
| yes | no | Section 3 stands as written. The work happened and the session denies it. |
| yes | yes | Section 3 needs rewriting. Finished rounds are flushed; only the in-flight round is lost, so the user loses a pending ask, not a record of completed work. |
| no | no | No orphan to fear *for that shape* — but only if the run's own `SETUP OK` line is present. A `SETUP DEGRADED` or `SETUP INVALID` run measured nothing and is just section 0 again. |
| no | yes | A fifth world nothing here predicted. Stop and write it up on its own. |

The harness prints `SETUP OK` / `DEGRADED` / `INVALID` before it parks, from
what it saw on the live stream, precisely so a bad setup cannot be mistaken for
a result.

**7.6.5 What it costs.** Two turns of two tool calls plus two readback turns:
about six provider calls on short prompts, so cents, and roughly four minutes
per case, nearly all of it the 75-second wait and model latency. It leaves two
files in the server's `/tmp` and four sessions on the server; the readback
sessions delete themselves, the two under test are the evidence and are printed
for the operator to keep or remove.

**7.6.6 What it still would not establish.** That a *partially* completed tool —
one killed mid-execution rather than after it returned — leaves a half-finished
side effect. Nothing here looks at that, and section 3 should not be read as
claiming it.
