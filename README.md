...
- **Connect** — see every MCP server your agent can reach, with the exact tool allowlist each one is held to, and add another from the phone with nothing but a token or an app password. The allowlist is the point: goose spells `available_tools` in snake_case among camelCase neighbours and has no `deny_unknown_fields`, so the natural camelCase spelling is *silently dropped* — and a dropped allowlist means every tool the server publishes is callable. So every extension is added *switched off*, then read back to assert the allowlist came back non-empty and identical, and only switched on if it did — an unrestricted extension is never live, not even for the round trip it would take to notice. Credentials go one way: written with `config/upsert` + `isSecret`, never read back (goose returns a clear prefix for a secret), never stored on the phone, and with no reveal control anywhere. Verification is a handshake — the extension is started in a session — not a read, and it always runs: with no chat open a throwaway session is created for it and deleted again, so "connected" means the service answered rather than that a token was filed away.
...
workspace, where the protocol and connection logic live. Coverage is reported to
Coveralls by CI using the built-in `GITHUB_TOKEN`; no account or repository
secret is needed.
