...
// The machine the Tailscale client has to be running on is named by
// `crate::shell::this_device`, which is where the local `tailscale_host` this
// screen used to carry went when #161 found nine more sentences that need the
// same noun. The reasoning is written there; the part that is this screen's
// own is why it matters here at all. This is the one screen a reader arrives at
// *because* the connection is not working, and being told to check an app on a
// device they are not holding is advice that cannot be followed.
    ...
    rsx! {
        ...
        main { class: "scroll settings",
            ...
            section { class: "about",
                ...
                p { "Reach a private server from anywhere with the Tailscale app enabled on {this_device(Shell::CURRENT)}." }
            }
        }
...
mod tests {
    ...
    use super::{crumb, this_device, Shell};
