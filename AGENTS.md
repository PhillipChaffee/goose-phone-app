...
the way the goose one takes `slow`, `notool`, `diff` and `nokind`. `diff` adds a
...
`nokind` adds a tool call with **no `kind` field at all**, which is the only way
to reach the tool card that draws a glyph instead of a word — every tool either
fake sent carried a kind, so `.tool-icon` had never been in a desktop frame
(#248).
...
used to do) and "Why, not what" (a comment earns its lines with a trade-off,
a measurement, or a contract, never a restatement of the signature) — and
those apply everywhere, prose and code. This repo adds four rules of its own:

- **Cite the issue, don't narrate it.** `#NNN` references are house currency,
  but only as a subordinate tag on a sentence that still works without them.
  "Both copies said `this phone` (#161)" makes an issue the sentence's
  subject and tombstones the comment; the issue is history, the constraint is
  not.
- **Doc comments carry what the signature cannot.** The wire's name for a
  field, what the other side sends there, defaults, degradation behavior,
  `# Errors` / `# Panics` sections where they apply. A doc line that only
  restates the item's name is the thing this repo's own lint policy once
  refused to compel (#327), so a bare introduce-and-stop is treated as a bug
  by review, not a format to satisfy.
- **A flagged identifier is config, not a rewrite.** When `doc_markdown`
  fires on a genuine house identifier, extend `doc-valid-idents` in
  `clippy.toml` (verbatim code spelling) rather than backticking or
  rewording the prose.
- **Nothing may display a number no server sends.** Dollar figures, latency
  percentiles, container counts, queue depth, per-turn durations and the
  user's own name are excluded in doc comments and examples the same way they
  are in the UI; `src/shell/desktop/inspector.rs` has the fullest version of
  that list.
...
The audit walks a **second grid** for the desktop shell: thirteen window sizes
at the one root a macOS build has, with four shell states — the nav's collapse,
the inspector's, and fullscreen, listed rather than multiplied — in place of the
text axis. A state says which grid it is on through its own key — a desktop dump
...
inside `assets/desktop/` — so nothing in Rust listens to a **DOM** resizable,
...
This file is the canonical project guide. `CLAUDE.md` is a one-line pointer to
it, for the tools that look for that name.
